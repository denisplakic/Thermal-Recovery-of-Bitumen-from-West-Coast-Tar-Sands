...
def lpm_model_initial(tO,O,tP,P,tS,S,tT,T,tW,W,tq,q):
    ''' Initial calibration of data to form pressure and temperature best-fit models using curve_fit and plots the models.
        ...
        Notes:
        ------
        Does not calibrate P0 and T0 as parameters - assumes constant values.
    ...
    def solve_lpm_pressure(t, a, b):
        ...
        for t0,t1 in zip(t_step_P[:-1],t_step_P[1:]):
            dPdt1 = lpm_pressure(Pm[-1]-1291.76, t0, a, b)
            Pp = Pm[-1] + dPdt1*(t1-t0)
            dPdt2 = lpm_pressure(Pp-1291.76, t1, a, b)
            Pm.append(Pm[-1] + 0.5*(t1-t0)*(dPdt2+dPdt1))
        return np.interp(t, t_step_P, Pm)

    # use curve_fit to find best model
    pars = curve_fit(solve_lpm_pressure, tP, P, [1,1])[0]
    ...
    ax.set_title('aP={:2.1e},   bP={:2.1e}'.format(*pars),size=14)

    plt.savefig("figures/lpm_pressure_initial.png")
    # plt.show()
    ...
    plt.savefig("figures/model_misfit_pressure_initial.png")
    # plt.show()
    ...
    def solve_lpm_temp(t, b, M0):
        ...
        for t0,t1 in zip(t_step_T[:-1],t_step_T[1:]):
            dTdt1 = lpm_temp(Tm[-1]-25, t0, b, M0)
            Tp = Tm[-1] + dTdt1*(t1-t0)
            dTdt2 = lpm_temp(Tp-25, t1, b, M0)
            Tm.append(Tm[-1] + 0.5*(t1-t0)*(dTdt2+dTdt1))
        return np.interp(t, t_step_T, Tm)

    # use curve_fit to find best model
    pars = curve_fit(solve_lpm_temp, tT, T, [0.001,100000])[0]
    ...
    ax.legend(prop={'size':14})
    ax.set_title('aP={:2.1e},   bP={:2.1e}\nbT={:2.1e},   M0={:2.1e}'.format(ap,bp,*pars),size=14)

    plt.savefig("figures/lpm_temp_initial.png")
    # plt.show()
    ...
    plt.savefig("figures/model_misfit_temperature_initial.png")
    # plt.show()